...
class OutputsLoader(object):
    ...
    def on_init_done(self, dlg):
        ...
        if dlg.ok_button.isEnabled():
            if dlg.output_type == 'hmaps':
                for imt_idx in range(dlg.imt_cbx.count()):
                    dlg.imt_cbx.setCurrentIndex(imt_idx)
                    for poe_idx in range(dlg.poe_cbx.count()):
                        dlg.poe_cbx.setCurrentIndex(poe_idx)
                        dlg.accept()
            else:
                dlg.accept()
        else:
    ...
    def load_output(self, calc, output):
        ...
        if output_type in (OQ_CSV_TO_LAYER_TYPES |
            ...
            dlg = OUTPUT_TYPE_LOADERS[output_type](
                # FIXME replace Mock() with actual viewer dock
    ...
    def test_load_outputs(self):
        ...
        try:
            selected_calc_id = int(self.selected_calc_id)
