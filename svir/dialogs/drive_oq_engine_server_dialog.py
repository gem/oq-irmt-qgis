...
class DriveOqEngineServerDialog(QDialog):
    ...
    def show_calc_list(self, calc_list):
        ...
        actions = ['Console', 'Remove', 'Outputs', 'Run Risk']
        self.ui.calc_list_tbl.setRowCount(len(calc_list))
        self.ui.calc_list_tbl.setColumnCount(len(selected_keys) + len(actions))
        ...
        for row, calc in enumerate(calc_list):
            ...
            for col, action in enumerate(actions, len(selected_keys)):
                ...
                QObject.connect(
                    ...
                    lambda calc_id=calc['id'], action=action: (
        ...
        headers = [key.capitalize()
                   for key in selected_keys] + ['' for action in actions]
    ...
    def get_calc_log(self, calc_id):
        calc_log_url = "%s/v1/calc/%s/log/:" % (self.hostname, calc_id)
    ...
    @pyqtSlot()
    def on_reload_outs_btn_clicked(self):
        # TODO
        pass
