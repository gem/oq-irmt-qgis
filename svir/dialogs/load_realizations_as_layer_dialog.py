...
class LoadRealizationsAsLayerDialog(LoadOutputAsLayerDialog):
    """
    Modal dialog to load realizations from an oq-engine output, as layer
    """

    def __init__(self, iface, viewer_dock, session, hostname, calc_id,
                 output_type='realizations', path=None, mode=None):
        assert output_type == 'realizations'
        LoadOutputAsLayerDialog.__init__(
            self, iface, viewer_dock, session, hostname, calc_id,
            output_type, path, mode)
        self.create_save_as_shp_ckb()
        self.setWindowTitle('Load realizations from CSV, as layer')
        self.adjustSize()
        self.set_ok_button()
