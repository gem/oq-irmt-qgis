...
class LoadLossesByEventAsLayerDialog(LoadOutputAsLayerDialog):
    """
    Modal dialog to load losses_by_event or agg_loss_table
    from an oq-engine output, as layer
    """

    def __init__(self, iface, viewer_dock, session, hostname, calc_id,
                 output_type=None, path=None, mode=None,
                 engine_version=None):
        assert output_type in ('losses_by_event', 'agg_loss_table')
        ...
        self.setWindowTitle(
            'Load %s as layer' % output_type)
        # self.create_num_sites_indicator()
        # NOTE: extracting 'losses_by_event/rlzs' also for agg_loss_table
        self.npz_file = extract_npz(
            session, hostname, calc_id, 'losses_by_event/rlzs',
